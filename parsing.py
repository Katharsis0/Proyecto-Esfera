...
def p_error(p):
    with open('syntax_errors.log', 'a') as f:
        f.write(f"Syntax error on line {p.lineno}: unexpected token {p.value}\n")
"""Sentencias"""
...
def p_def_rule(p):
    ''' reservedkey : DEF LP ID COMMA TYPE RP SEMICOLON
                 ...
                 | DEF LP ID COMMA TYPE COMMA value RP SEMICOLON
    '''
...
# def p_print_rule(p):
#     ''' expression : PRINT LP STRING RP SEMICOLON
#                     |PRINT LP ID RP SEMICOLON
#     '''
...
def p_call_rule(p):
    ''' reservedkey : CALL LP ID RP SEMICOLON
    '''
    p[0]=p[3]
